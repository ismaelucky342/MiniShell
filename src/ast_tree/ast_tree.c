...
/*   Updated: 2024/12/05 12:00:58 by ismherna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * Compares the token string with redirection symbols and sets the redirection type
 * in the redirection token.
 * 
...
static void red_compare(t_list *tkn_lst, t_list **tmp)
{
    if (ft_strncmp("<", ((t_token *)tkn_lst->content)->str, 2) == 0)
        ((t_redirection_token *)(*tmp)->content)->redir_type = M_INFILE;
    else if (ft_strncmp(">", ((t_token *)tkn_lst->content)->str, 2) == 0)
        ((t_redirection_token *)(*tmp)->content)->redir_type = M_OUT;
    else if (ft_strncmp("<<", ((t_token *)tkn_lst->content)->str, 3) == 0)
        ((t_redirection_token *)(*tmp)->content)->redir_type = M_HEREDOC;
    else if (ft_strncmp(">>", ((t_token *)tkn_lst->content)->str, 3) == 0)
        ((t_redirection_token *)(*tmp)->content)->redir_type = M_CONCAT_OUT_ST;
}
...
static int set_file_info(t_list *tkn_lst, t_tree_node *current)
{
    ...
    tmp->content = ft_calloc(1, sizeof(t_redirection_token));
    ((t_redirection_token *)tmp->content)->file_name = ft_strdup(((t_token *)
...
/**
 * Counts the number of arguments in the token list until a PIPE token is encountered.
 * 
...
/**
 * Fills the command list in the abstract syntax tree (AST) by processing the token list.
 * 
 * @param begin The beginning of the token list.
